...
def migrate_schema() -> None:
    """Adds columns introduced after the initial release, without touching existing data."""
    with closing(sqlite3.connect(DB_PATH)) as conn:
...
def ensure_admin_account() -> None:
    """Creates the first administrator account if none exists yet."""
    with closing(sqlite3.connect(DB_PATH)) as conn:
        conn.row_factory = sqlite3.Row
        ...
        password = os.environ.get("ADMIN_PASSWORD", "ChimieAdmin2026!")
        ...
        if existing:
            ...
            conn.execute(
                """INSERT INTO users(full_name, username, password_hash, role, bio, created_at, email, is_admin)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    "Administrator",
                    username,
                    ...
                    "profesor",
                    "Cont de administrare platformă.",
                    now,
                    "",
                    1,
                ),
...
def admin_required(view):
    ...
    def wrapped(*args, **kwargs):
        ...
        if not user["is_admin"]:
            flash("Această zonă este rezervată administratorului.", "danger")
            return redirect(url_for("dashboard"))
        return view(*args, **kwargs)
...
def register():
    ...
    if request.method == "POST":
        ...
        if len(full_name) < 3:
            ...
            user_id = execute_db(
                "INSERT INTO users(full_name, username, password_hash, role, bio, created_at, email) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (
                    full_name,
                    username,
                    generate_password_hash(password),
    ...
    if query:
        ...
        users = query_all(
            """SELECT * FROM users
               ...
               ORDER BY is_admin DESC, role, full_name COLLATE NOCASE""",
            (like, like, like),
        )
    else:
        users = query_all("SELECT * FROM users ORDER BY is_admin DESC, role, full_name COLLATE NOCASE")
    stats = {
        "total": len(users) if query else query_one("SELECT COUNT(*) AS c FROM users")["c"],
        ...
        "admins": query_one("SELECT COUNT(*) AS c FROM users WHERE is_admin = 1")["c"],
    }
    return render_template("admin.html", users=users, stats=stats, query=query)
...
def admin_create_user():
    ...
    if len(full_name) < 3 or len(username) < 3 or role not in {"profesor", "elev"}:
        flash("Completează numele, utilizatorul și rolul corect.", "danger")
    ...
    flash(f"Cont creat pentru {username}. Parolă: {password}", "success")
    return redirect(url_for("admin_panel"))
...
def admin_update_user(user_id: int):
    ...
    if len(full_name) < 3 or len(username) < 3 or role not in {"profesor", "elev"}:
        flash("Completează numele, utilizatorul și rolul corect.", "danger")
        return redirect(url_for("admin_panel"))
    conflict = query_one("SELECT id FROM users WHERE username = ? AND id != ?", (username, user_id))
    if conflict:
        flash("Acest nume de utilizator este deja folosit de alt cont.", "danger")
        return redirect(url_for("admin_panel"))
    ...
    flash(f"Contul {username} a fost actualizat.", "success")
    return redirect(url_for("admin_panel"))
...
def admin_reset_password(user_id: int):
    ...
    new_password = request.form.get("password", "").strip() or generate_temp_password()
    if len(new_password) < 4:
        flash("Parola trebuie să aibă cel puțin 4 caractere.", "danger")
        return redirect(url_for("admin_panel"))
    ...
    flash(f"Parolă nouă pentru {target['username']}: {new_password}", "success")
    return redirect(url_for("admin_panel"))
...
def admin_toggle_admin(user_id: int):
    ...
    if target["id"] == actor["id"] and target["is_admin"]:
        admin_count = query_one("SELECT COUNT(*) AS c FROM users WHERE is_admin = 1")["c"]
        if admin_count <= 1:
            flash("Nu poți elimina singurul cont de administrator.", "danger")
            return redirect(url_for("admin_panel"))
    execute_db("UPDATE users SET is_admin = ? WHERE id = ?", (0 if target["is_admin"] else 1, user_id))
    flash("Statutul de administrator a fost actualizat.", "success")
    return redirect(url_for("admin_panel"))
...
def admin_delete_user(user_id: int):
    ...
    if target["is_admin"]:
        admin_count = query_one("SELECT COUNT(*) AS c FROM users WHERE is_admin = 1")["c"]
        if admin_count <= 1:
            flash("Nu poți șterge singurul cont de administrator.", "danger")
            return redirect(url_for("admin_panel"))
    execute_db("DELETE FROM users WHERE id = ?", (user_id,))
    flash(f"Contul {target['username']} a fost șters.", "success")
    return redirect(url_for("admin_panel"))
